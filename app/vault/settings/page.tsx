...
export default function SettingsPage() {
  ...
  async function loadSettings() {
    ...
    const { data } = await supabase
      .from("user_profiles")
      .select("auto_lock_seconds, emergency_contact_email, hidden_vault_hash, dead_man_switch_enabled, dead_man_inactivity_days, last_active_at")
      .eq("user_id", user.id)
      .single();
                    ...
                    {securityLogs.map((log) => (
                      ...
                      >
                        <div>
                          ...
                          {log.user_agent && (
                            <p className="mt-1 truncate text-xs text-muted-foreground" style={{ maxWidth: 400 }}>
                              {log.user_agent}
                            </p>
                          )}
                        </div>
